...
object OntContructorMain {
  def main(args: Array[String]): Unit = {
    ...
    val ONTOLOGYURI="http://www.semanticweb.org/center/ontologies/2018/10/"
    ...
    triplets.foreach(f=>{
      ...
      val farr=f.split(",")
      val x = farr(1).replaceAll(" ","")
      val sub = df.getOWLNamedIndividual(farr(0), pm)

      if(farr(3)=="Obj")
       {
         val pred=df.getOWLObjectProperty(x,pm)
        ...
        {
          val pred=df.getOWLDataProperty(x,pm)
    ...
    val os = new FileOutputStream("data/Disease.owl")
